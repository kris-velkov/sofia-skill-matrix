...
export default function AddEmployeePage() {
  ...
  const { isAuthenticated, isAdmin, isLoading } = useAuth() // Get auth status
  ...
  useEffect(() => {
    if (!isLoading && (!isAuthenticated || !isAdmin)) {
      ...
      router.push("/employees") // Redirect if not authenticated or not admin
    }
  ...
  useEffect(() => {
    if (state?.success) {
      ...
      router.push("/employees") // Redirect on successful addition
  ...
  if (isLoading || !isAuthenticated || !isAdmin) {
    // Show loading or redirecting message while auth status is being determined
    return (
      ...
      <main className="flex-1 p-4 md:p-6">
        <div className="max-w-4xl mx-auto grid gap-6">
          <div className="flex items-center gap-4">
            ...
            <h2 className="text-2xl font-bold text-gray-800 dark:text-gray-100">Add New Employee</h2>
          </div>
            ...
            <Card className="p-6 shadow-md rounded-xl">
              ...
              <CardContent className="p-0 grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
                <div className="grid gap-1">
                  <Label htmlFor="name" className="text-gray-500 dark:text-gray-400 text-sm">
                    Full Name
                  </Label>
                ...
                {/* Hidden input for skills - will be initialized by Server Action */}
