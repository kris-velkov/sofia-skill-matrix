...
import { COMPETENCY_LEVELS } from "@/lib/data" // Assuming COMPETENCY_LEVELS is still needed here

interface DonutChartProps {
  data: { skillName: string; level: number; count: number; percentage: number; color: string }[]
  onSegmentClick?: (skillName: string, level: number) => void
  selectedSkill?: string | null
  selectedLevel?: number | null
}

export function DonutChart({ data, onSegmentClick, selectedSkill, selectedLevel }: DonutChartProps) {
  const svgRef = useRef<SVGSVGElement | null>(null)
  const [dimensions, setDimensions] = useState({ width: 0, height: 0 })

  const getCompetencyColor = useCallback((level: number) => {
    return COMPETENCY_LEVELS.find((c) => c.grade === level)?.color || "bg-gray-400"
  }, [])
  ...
  const drawChart = useCallback(() => {
    ...
    const arc = d3
      .arc<d3.PieArcDatum<{ skillName: string; level: number; count: number }>>()
    ...
    arcs
      ...
      .classed("opacity-100", (d) => {
        ...
        return d.data.skillName === selectedSkill && d.data.level === selectedLevel
      })
      ...
      .text((d) => `${d.data.skillName} (Level ${d.data.level}): ${d.data.count} employees`)
  ...
  }, [data, dimensions, onSegmentClick, selectedSkill, selectedLevel, getCompetencyColor])
